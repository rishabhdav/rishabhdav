<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0B1120,50:0F172A,100:1E293B&height=200&section=header&text=Rishabh%20Tripathi&fontSize=44&fontColor=E2E8F0&fontAlignY=38&desc=Java%20Backend%20Developer&descAlignY=54&descSize=20&descColor=38BDF8&animation=fadeIn" width="100%"/>

<a href="https://www.linkedin.com/in/rishabh-tripathi-096833284/">
<img src="https://readme-typing-svg.demolab.com/?font=JetBrains+Mono&size=22&duration=2800&pause=1100&color=38BDF8&center=true&vCenter=true&width=700&lines=Building+Reliable+Backend+Systems;Java+%7C+Spring+Boot+%7C+REST+APIs+%7C+MySQL;500%2B+DSA+Problems+Solved;Open+to+SDE+%2F+Backend+Engineer+Roles" />
</a>

<br><br>

<img src="https://img.shields.io/badge/Open%20to%20Work-SDE%20%2F%20Backend-38BDF8?style=flat-square&labelColor=0B1120"/>

<img src="https://img.shields.io/badge/Location-India-38BDF8?style=flat-square&labelColor=0B1120"/>

<img src="https://komarev.com/ghpvc/?username=rishabhdav&label=Profile%20Views&color=38BDF8&style=flat-square"/>

</div>

---

# 👋 About Me

<table>
<tr>

<td width="60%">

### Hi, I'm Rishabh 👋

🎓 Final Year B.Tech Computer Science Student

💻 Java Backend Developer

🚀 Passionate about Backend Engineering, REST APIs and Scalable Applications

🔥 Solved **500+ DSA Problems**

🌱 Currently Learning

- System Design
- Microservices
- Docker

🎯 Looking for

- SDE-1
- Backend Engineer
- Java Developer

</td>

<td width="40%">

```yaml
Role: Java Backend Developer

Language:
  - Java

Backend:
  - Spring Boot
  - Spring Security
  - Hibernate
  - REST API

Database:
  - MySQL

Learning:
  - Docker
  - System Design
  - Microservices

DSA:
  500+
```

</td>

</tr>
</table>

---

# 🛠 Tech Stack

<div align="center">

| Category | Skills |
|-----------|--------|
| Languages | Java • JavaScript • HTML • CSS |
| Backend | Spring Boot • Spring Security • Hibernate • REST APIs • JWT |
| Frontend | React |
| Database | MySQL |
| Tools | Git • GitHub • IntelliJ • VS Code • Maven |
| Learning | Docker • System Design • Microservices |

</div>

---

# 📊 GitHub Dashboard

<div align="center">

<!-- TODO: swap github-readme-stats.vercel.app for your self-hosted Vercel instance once deployed, to avoid public-instance rate limits -->
<img height="170" src="https://github-readme-stats.vercel.app/api?username=rishabhdav&show_icons=true&theme=tokyonight&hide_border=true"/>

<img width="48%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=rishabhdav&layout=compact&theme=tokyonight&hide_border=true"/>

<img width="97%" src="https://github-readme-activity-graph.vercel.app/graph?username=rishabhdav&theme=react-dark&hide_border=true"/>

</div>

---

# 🚀 Featured Projects

## 🛒 E-Commerce Platform

> Full Stack E-Commerce Application built using Spring Boot & React.

### ✨ Features

- JWT Authentication
- Spring Security
- Product Management
- Shopping Cart
- Order Management
- Stripe Payment Integration
- Admin Dashboard
- Responsive UI
- REST APIs

### 🏗 Tech Stack

`Java` `Spring Boot` `Spring Security` `JWT` `Hibernate` `MySQL` `React` `Redux` `Stripe`

---

## 📝 Blog Application REST API

> Backend-first blogging platform with secure REST APIs.

### ✨ Features

- CRUD Operations
- JWT Authentication
- Role Based Authorization
- Swagger Documentation
- Global Exception Handling
- Validation
- Pagination & Sorting

### 🏗 Tech Stack

`Java` `Spring Boot` `Hibernate` `MySQL` `Swagger` `JWT`

---

## 📊 SQL Data Warehouse Analytics

> Star Schema Data Warehouse with ETL Pipeline.

### ✨ Features

- ETL Pipeline
- Star Schema
- Fact & Dimension Tables
- T-SQL Reporting
- Query Optimization

### 🏗 Tech Stack

`SQL` `T-SQL` `ETL` `Data Warehouse`

---

# 💻 Coding Profiles

<div align="center">

| Platform | Profile |
|----------|---------|
| 🟠 LeetCode | https://leetcode.com/u/rishabhtri/ |
| 🟢 GeeksforGeeks | https://www.geeksforgeeks.org/profile/rishabhtricywd |
| 🟤 CodeChef | https://www.codechef.com/users/bright_card_63 |
| 🟩 HackerRank | https://www.hackerrank.com/profile/rishabhtripath49 |
| 🔵 Codolio | https://codolio.com/profile/mHlpjSWV |

<br>

<a href="https://codolio.com/profile/mHlpjSWV">

<img src="https://codolio.com/api/og/mHlpjSWV" width="650"/>

</a>

</div>

---

# 🏆 Achievements

- 🚀 500+ DSA Problems Solved
- 💻 3+ End-to-End Projects
- 🔐 Spring Security & JWT
- 🌐 REST API Development
- 📖 System Design Learner
- 🐳 Learning Docker & Microservices

---

# 📚 Current Learning

```text
✅ Core Java

✅ Spring Boot

✅ Hibernate

✅ Spring Security

✅ REST APIs

✅ MySQL

🔄 System Design

🔄 Microservices

🔄 Docker
```

---

# 🌐 Connect With Me

<div align="center">

<a href="https://www.linkedin.com/in/rishabh-tripathi-096833284/">

<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>

</a>

<a href="https://github.com/rishabhdav">

<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>

</a>

<a href="https://leetcode.com/u/rishabhtri/">

<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>

</a>

<a href="https://www.geeksforgeeks.org/profile/rishabhtricywd">

<img src="https://img.shields.io/badge/GeeksforGeeks-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white"/>

</a>

</div>

---

<div align="center">

### ⭐ Thanks for visiting my profile!

*"Building scalable backend systems one commit at a time."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1E293B,50:0F172A,100:0B1120&height=200&section=footer"/>

</div>
